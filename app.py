...
def _build_lead_prompt(*, client_name, client_url, client_city, client_product, request_summary,
  ...
  "slides": {
    ...
    "next_step": {
      "samples": [
        {"name": "ТОЧНОЕ имя из АКТУАЛЬНОГО ПРАЙСА VERDE TECH 2026 выше (не выдумывай). 1-3 позиции максимум.", "qty": "100 г для теста (или больше если позиция уже в виде бутыли/большой фасовки)", "purpose": "Зачем эта позиция в тесте (1 короткая фраза, увязанная с client_product)"},
        {"name": "Имя 2 — другая категория из БЗ если уместно (например к функциональной смеси добавить ароматизатор)", "qty": "100 г", "purpose": "..."},
        {"name": "Опционально 3-я позиция", "qty": "...", "purpose": "..."}
      ],
      "first_step_includes": [
        "Образцы по 100 г каждой позиции бесплатно — отправим СДЭКом за 5-7 дней",
        "Рекомендации технолога по применению (дозировка, технологический режим)",
        "Минимальные партии при последующем заказе — от 25 кг",
        "Документы по продукту (СГР, СоС, спецификации) — если требуются"
      ],
      "from_client_needed": [
        "Адрес для отправки образца",
        "Продукт клиента для теста (рецептура / ТУ если есть)",
        "Технология производства (этап внесения, температурный режим)",
        "Целевая себестоимость, если есть"
      ],
      "cta": "Согласовать состав тестовой посылки и адрес. БОНУС: если дадите обратную связь по образцу в течение 5 дней — 5% скидка на первый заказ."
    },
    "bonus_competitors": {
...
⛔ ЖЁСТКОЕ ПРАВИЛО: МЫ ПРОДАЁМ СВОЁ, А НЕ СОВЕТУЕМ КЛИЕНТУ ДЕЛАТЬ НОВЫЕ ПРОДУКТЫ
Тебе ЗАПРЕЩЕНО предлагать клиенту «выпустите новую линейку X», «добавьте в ассортимент Y», «создайте продукт Z» — это работа их маркетолога, не наша.
Мы — поставщик ингредиентов. Каждое наше «предложение» — это КОНКРЕТНАЯ ПОЗИЦИЯ ИЗ НАШЕГО АССОРТИМЕНТА (см. БЗ выше) для УЛУЧШЕНИЯ или РАСШИРЕНИЯ их ТЕКУЩЕЙ продукции.

✅ ХОРОШО: «Для вашей линейки пельменей подойдёт V Эмультек Премиум (стабилизатор) — увеличит сочность фарша на 10-15%».
❌ ПЛОХО: «Запустите новый продукт — пельмени с грибами, сейчас тренд».
✅ ХОРОШО: «К вашим колбасам предлагаем V Колор RP — натуральный краситель кармином, заменит синтетику».
❌ ПЛОХО: «Сделайте линейку колбас с натуральными красителями».

Если в БЗ для ситуации клиента нет ни одной релевантной позиции — НЕ выдумывай. Скажи: «Под этот конкретный запрос в нашем прайсе пока нет готового решения — наш R&D готов разработать под вашу спецификацию».

⛔ ЖЁСТКОЕ ПРАВИЛО ДЛЯ next_step.samples (Слайд 6):
Каждая позиция в samples ОБЯЗАНА быть из АКТУАЛЬНОГО ПРАЙСА VERDE 2026 (см. БЗ выше).
...
def _generate_kp_lead(data):
    ...
    # T9: конкретные позиции для образца — список объектов {name, qty, purpose}
    next_samples = next_step_data.get('samples') or []
    if isinstance(next_samples, list):
        # Чистим: оставляем только записи где есть имя
        next_samples = [
            {
                'name':    str(s.get('name', '')).strip(),
                'qty':     str(s.get('qty', '100 г')).strip() or '100 г',
                'purpose': str(s.get('purpose', '')).strip(),
            }
            for s in next_samples if isinstance(s, dict) and (s.get('name') or '').strip()
        ][:5]
    else:
        next_samples = []
